...
<details class="endpoints">
    ...
    <ul>
        ...
        <li><a href="/h3stats" target="_blank">/h3stats</a>
            ...
            for connections that have already been reaped.
            To populate: open
...
$test->get('/h3stats', function () use ($test) {
    /*
        ...
        With ?keep=1, the response also includes the listener's
        ring buffer of stats from connections that have already
        been reaped. Useful for inspecting the final state of a
        connection that completed its work and got cleaned up
        before /h3stats could be hit. Each reaped entry has a
        'reason' field ('closed' for orderly shutdown,
        'stale_handshake' for forced reap of an abandoned
        handshake) and a 'reaped_at' microtime float.
     */
    $test->header("Content-Type: application/json");
    $stats = [];
    $reaped = [];
    $include_reaped = !empty($_GET['keep']);
