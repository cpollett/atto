 ...
 *
 * Phase 11b unit tests for H3NativeListener -- exercises the
 * RFC 9002 RTT smoothing (smoothed_rtt, rttvar, min_rtt) and
 * loss-detection timer (basePto, setLossDetectionTimer,
 * onLossDetectionTimeout) introduced in commit "phase 11b
 * (h3-native): RTT smoothing + PTO timer". Covers the first-
 * sample seed, the EWMA blend on subsequent samples, the
 * min_rtt floor against bogus ack_delay, the kInitialRtt
 * fallback when no RTT sample exists, exponential backoff on
 * pto_count, and the PING-on-timeout behaviour.
 *
 * Run from the repo root:
 *     php "examples/18 HTTP Benchmarks/test_phase11b.php"
 *
